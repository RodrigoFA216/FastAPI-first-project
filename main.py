...
def implementacion():
    ...
    salida = n.activacion(1, .5)
    return [salida]


@app.get('/ML/neurona/neurona', tags=['Neurona'])
def testNeuronal():
    res = implementacion()
    return [res]
...
def detectFunction(min: float = Body(), max: float = Body()):
    ...
    # numpy configura la generación de datos random con una semilla 42
    np.random.seed(42)
    # genera datos de 100 filas por dos columnas
    ...
    # opción 1
    y_predict_dict = {i: x.tolist()
                      for i, x in enumerate(y_pred) if x.tolist() == -1}
